...
func GetCommentList(pageSize int, pageNum int) ([]Comment, int64, int) {
	...
	err = db.Model(&Comment{}).Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Select("comments.id, articles.title,user_id,article_id, users.username, comments.content, comments.status,comments.created_at,comments.deleted_at").Joins("LEFT JOIN articles ON comments.article_id = articles.id").Joins("LEFT JOIN users ON comments.user_id = users.id").Scan(&commentList).Error
	if err != nil {
...
func GetCommentListFront(id int, pageSize int, pageNum int) ([]Comment, int64, int) {
	...
	var total int64
	db.Find(&Comment{}).Where("article_id = ?", id).Where("status = ?", 1).Count(&total)
	err = db.Model(&Comment{}).Limit(pageSize).Offset((pageNum-1)*pageSize).Order("Created_At DESC").Select("comments.id, articles.title, user_id, article_id, users.username, comments.content, comments.status,comments.created_at,comments.deleted_at").Joins("LEFT JOIN articles ON comments.article_id = articles.id").Joins("LEFT JOIN users ON comments.user_id = users.id").Where("article_id = ?",
